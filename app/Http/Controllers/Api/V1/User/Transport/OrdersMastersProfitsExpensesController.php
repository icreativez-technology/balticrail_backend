...
#Model
use App\Models\Transport\OrdersMastersProfitsExpenses;

#Request 
use App\Http\Requests\User\Transport\OrdersMastersProfitsExpenses\StoreRequest;
use App\Http\Requests\User\Transport\OrdersMastersProfitsExpenses\UpdateRequest;
...
class OrdersMastersProfitsExpensesController extends Controller
{
    ...
    {
        $data = OrdersMastersProfitsExpenses::where('order_id','=',$id)->get();
        $response = $data;
    ...
    public function store(StoreRequest $request)
    {
        try 
        {
            ...
            $record = new OrdersMastersProfitsExpenses();
            $record->order_id = isset($value['order_id']) ? $value['order_id'] : 0;
            $record->expense_code_id = isset($value['expense_code_id']) ? $value['expense_code_id'] : 0;
    ...
    public function update(UpdateRequest $request,$id)
    {
        try 
        {
            ...
            $record = OrdersMastersProfitsExpenses::find($id);
    ...
    {
        $record = OrdersMastersProfitsExpenses::where('id','=',$id)->first();
